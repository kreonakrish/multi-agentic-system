...
CREATE TABLE `messages` (
  ...
  `interaction_type` enum('direct','workflow','broadcast') DEFAULT 'direct',
  `processed_message` text,
  `status` enum('pending','delivered','processed') DEFAULT 'pending',
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `sender_id` (`sender_id`),
  KEY `receiver_id` (`receiver_id`),
  CONSTRAINT `messages_ibfk_1` FOREIGN KEY (`sender_id`) REFERENCES `agents` (`id`),
  CONSTRAINT `messages_ibfk_2` FOREIGN KEY (`receiver_id`) REFERENCES `agents` (`id`)

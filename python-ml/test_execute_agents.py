...
class TestAgentExecutor(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        try:
            ...
            # 7. Delete tool
            self.cursor.execute("DELETE FROM tools WHERE id = %s", (self.tool_id,))
            logger.info("Deleted tool")
            
            # 8. Finally delete team
            self.cursor.execute("DELETE FROM teams WHERE id = %s", (self.team_id,))
            logger.info("Deleted team")
            
            self.conn.commit()
            logger.info(f"Cleanup completed successfully in {time.time() - start_time:.2f} seconds")
